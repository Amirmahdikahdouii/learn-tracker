...
- **Components** ([frontend/src/components/](frontend/src/components/)): `Dashboard` (course list/grid, embeds `ActivityHeatmap`), `CourseDetail` (chapter/section drill-down + toggling completion), `CourseEditor` (create/edit a course's chapters, sections, and cover image URL), `ActivityHeatmap` (GitHub-style completion heatmap), `Accordion`/`SectionItem`/`ProviderBadge` are smaller presentational pieces reused by the above. `ProviderBadge` only has icon/color styling for a fixed set of known provider strings (matched via `switch`); unrecognized providers (including free-text ones a user types in `CourseEditor`) fall back to a generic gray badge.
- **Demo mode** (`demo` branch, deployed via `.github/workflows/deploy-demo.yml`): when built with `VITE_DEMO_MODE=true`, [frontend/src/data/demoCourses.ts](frontend/src/data/demoCourses.ts) exports `DEMO_COURSES`, which `courseStore`'s `seedDemoIfNeeded`/`resetDemoData` actions push into `courses` on first load (see `main.ts`). Each demo section's `completedAt` is computed with a local `daysAgo(n)` helper relative to `new Date()` at module-evaluation time (i.e. whenever the page loads) rather than hardcoded — this keeps the activity heatmap looking freshly populated (recent + mid-range + older activity, occasional multi-completion days) no matter when the live demo is visited, instead of the seed data going stale/scrolling out of the 52-week window.
