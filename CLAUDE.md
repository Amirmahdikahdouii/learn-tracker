...
## utils/arvanCloudAcademyCourseExtractor

Same idea again, for academy.arvancloud.ir course pages (a WordPress + Elementor + LearnDash site). Unlike hamrah.academy, this site is fully server-rendered — the chapter/lesson outline is plain static HTML — so `main.py` goes back to BeautifulSoup/CSS-class scraping like the maktabkhoone extractor. Chapters live inside an Elementor accordion widget scoped by a `course-headline` class (each `.elementor-accordion-item`'s `.elementor-accordion-title` is a chapter, its `.elementor-tab-content li a` links are sections); the selector deliberately targets `course-headline` specifically rather than any `elementor-widget-accordion`, since the page also has an unrelated FAQ accordion with the identical widget structure just below the chapters. `imageUrl` is read from the page's `og:image` meta tag, which is a generic site-wide banner rather than a real per-course cover image. Per-lesson metadata (duration, video/quiz type, free/locked) isn't present on the course page and so isn't captured. See [utils/arvanCloudAcademyCourseExtractor/README.md](utils/arvanCloudAcademyCourseExtractor/README.md) for setup/usage.
